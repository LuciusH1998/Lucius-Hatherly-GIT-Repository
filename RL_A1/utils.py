...
def discrete_2d_convolution(image, kernel):
    ...
    image_padded = np.pad(image, ((pad_height, pad_height), 
                                  (pad_width, pad_width)), 
                                  mode='constant', 
                                  constant_values=0)
    # TODO: perform the convolution operation
    produced_output = np.zeros((image_height, image_width))
    # Iterating through image height and width
    # Performing the convolution operation (cross-correlation which is identical as kernel is not flipped)
    for i in range(image_height):
        for j in range(image_width):
            area = image_padded[i:i+kernel_height, j:j+kernel_width] 
            produced_output[i,j] = np.sum(area*kernel)
    # Return the output 
...
class DiceCELoss(nn.Module):
    ...
    def forward(self, logits, targets):
        # Defining dice loss, ce loss, and sum of the two losses 
