...
for col in numeric_cols:
    ...
    # Removing these outlier columns 
...
# Define parameter grid (narrow around your current best values)
param_grid = {
...
# %%
# Performing more substantive feature engineering to show adding or removing columns does not really improve feature engineering
from sklearn.preprocessing import StandardScaler
from sklearn.pipeline import Pipeline
from sklearn.ensemble import RandomForestClassifier
from sklearn.metrics import accuracy_score, classification_report
...
X_train["alcohol_eff"] = df_train["alcohol_ABV"]/(df_train["fermentation_strength"] + 1e6)
X_val["alcohol_eff"] = df_test["alcohol_ABV"]/(df_test["fermentation_strength"] + 1e6)
df_test_fe = df_test.copy()
df_test_fe["alcohol_eff"] = df_test_fe["alcohol_ABV"]/(df_test["fermentation_strength"] + 1e6)
...
X_train = X_train.drop(columns=["pH", "fermentation_strength"], errors="ignore")
X_val = X_val.drop(columns=["pH", "fermentation_strength"], errors="ignore")
df_test_fe = df_test_fe.drop(columns=["pH", "fermentation_strength"], errors="ignore")

# Defining top random forest models
...
print("Evaluating the Random Forest Model with additional feature engineering: ")
...
# Generating Kaggle file from the enhanced feature engineering data set
