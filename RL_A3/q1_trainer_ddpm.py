...
class Trainer:
    ...
    def sample(self, n_steps=None, noise=None, set_seed: bool = False):
        ...
        with torch.no_grad():
            ...
            for t_ in tqdm(range(n_steps)):
                ...
                # ==========================
                # Define t and x
                t = torch.full((x.shape[0],), n_steps - 1 - t_, device=self.args.device, dtype=torch.long)
    ...
    def generate_intermediate_samples(
        ...
        for step in tqdm(range(1, n_steps + 1, 1)):
            ...
            # TODO: Write your code here
            # ==========================
            t = torch.full((x.shape[0],), n_steps - step, device=self.args.device, dtype=torch.long)
            x = self.diffusion.p_sample(x, t)
            # STUDENT TODO END
